...
namespace DataClass
{
    public class OrderwiseRpt
    {
        ...
        public DataTable RptLoadDetailReport(DateTime fDt, DateTime tDt, string timeType, DateTime time, int locationId)
        {
            ...
            string constring1 = "";
            //constring1 = "Server=" + CommonView.DBServer + "; Database = " + CommonView.DataBase + ";UID=root;Pwd=" + CommonView.Pwd + ";Port=" + CommonView.Port + " ;SslMode=none;AllowPublicKeyRetrieval=True;Connect Timeout=30;Convert Zero Datetime=True;Charset=utf8;";
            constring1 = "Server=" + CommonView.DBServer + "; Database = " + CommonView.DataBase + ";UID=" + CommonView.UId + ";Pwd=" + CommonView.Pwd + ";Port=" + CommonView.Port + " ;SslMode=none;AllowPublicKeyRetrieval=True;Connect Timeout=30;Convert Zero Datetime=True;Charset=utf8;";
            ...
            using (MySqlConnection conn = new MySqlConnection(constring1))
            {
                ...
                if (timeType == "ALL")
                {
                    ...
                    // Old row: qry += " '') AS PaymentMode,1 AS OrdNo,O.CustomerId,'' as Pincode,py.metadata,'' as PaymentStatus, O.customFieldsLoyaltypointsused AS RewardPointUsed,O.couponCodes AS CouponCode,om.note";
                    // New row: Added py.method as PaymentMethod column (Line 313)
                    qry += " '') AS PaymentMode,1 AS OrdNo,O.CustomerId,'' as Pincode,py.metadata,py.method as PaymentMethod,'' as PaymentStatus, O.customFieldsLoyaltypointsused AS RewardPointUsed,O.couponCodes AS CouponCode,om.note,COALESCE(O.customFieldsInvoiceno,'') AS customFieldsInvoiceno";
                    qry += " FROM " + CommonView.DataBase +".`order` O";
                    qry += " INNER JOIN " + CommonView.DataBase +".order_line OL ON O.id = OL.orderId";
                    qry += " INNER JOIN " + CommonView.DataBase +".product_variant_translation P ON OL.productVariantId = P.id";
                    qry += " INNER JOIN " + CommonView.DataBase + ".product_variant pv ON pv.id = OL.productVariantId";
                    ...
                    if (locationId == 4)
                    {
                        qry += " AND (st.name = 'Tomorrow Morning Delivery (Rs.40 incl.Tax)' or st.name='Day After Tomorrow – Morning Delivery (Rs.10 incl. tax)') ";
                    }
                    else if (locationId == 6)
                    {
                        qry += " AND (st.name = 'Tomorrow Morning Delivery (Rs.50 incl.Tax)' or st.name='Day After Tomorrow – Morning Delivery (Rs.10 incl. tax)') ";
                    ...
                    if (locationId == 4 || locationId == 6)
                    {
                        ...
                        qry += " COALESCE(";
                        qry += " JSON_UNQUOTE(JSON_EXTRACT(CAST(he.data AS JSON), '$.note')),";
                        // Old row: qry += " '') AS PaymentMode,2 AS OrdNo,O.CustomerId,'' as Pincode,py.metadata,'' as PaymentStatus, O.customFieldsLoyaltypointsused AS RewardPointUsed,O.couponCodes AS CouponCode,om.note";
                        // New row: Added py.method as PaymentMethod column (Line 370)
                        qry += " '') AS PaymentMode,2 AS OrdNo,O.CustomerId,'' as Pincode,py.metadata,py.method as PaymentMethod,'' as PaymentStatus, O.customFieldsLoyaltypointsused AS RewardPointUsed,O.couponCodes AS CouponCode,om.note,COALESCE(O.customFieldsInvoiceno,'') AS customFieldsInvoiceno";
                        qry += " FROM " + CommonView.DataBase + ".`order` O";
                        qry += " INNER JOIN " + CommonView.DataBase + ".order_line OL ON O.id = OL.orderId";
                        qry += " INNER JOIN " + CommonView.DataBase + ".product_variant_translation P ON OL.productVariantId = P.id";
                        qry += " INNER JOIN " + CommonView.DataBase + ".product_variant pv ON pv.id = OL.productVariantId";
                        ...
                        if (locationId == 4)
                        {
                            qry += " and (st.name = 'Tomorrow Evening Delivery (Rs.20 incl.Tax)' or st.name='Day After Tomorrow – Evening Delivery (Rs.10 incl. tax)') ";
                        }
                        else if (locationId == 6)
                        {
                            qry += " and (st.name = 'Tomorrow Evening Delivery (Rs.30 incl.Tax)' or st.name='Day After Tomorrow – Evening Delivery (Rs.10 incl. tax)') ";
                ...
                else if (timeType == "GREATER")
                {
                    ...
                    // New row: Added py.method as PaymentMethod column (Line 450)
                    qry += " SELECT O.iD,SKU AS PCODE,DENSE_RANK() OVER (ORDER BY Code) as SNo,CODE,CAST(O.updatedAt AS DATE)Date,CAST(O.updatedAt AS TIME)TIME,ShippingAddress as BillingAddress,'' as Customer,'' as Address,'' as CellNo,trim(p.NAME) as ItemName,'' as TamilName,count(LineId) as Qty,s.ListPrice/100 as Rate, (Count(LineId)*(s.Listprice/100)) as as Total,subTotalWithTax/100 as SubTotal,shippingWithTax/100 as Shipping,(subTotalWithTax+shippingWithTax)/100 as NetTotal,st.Name as DeliveryTime,COALESCE((SELECT CAST(JSON_UNQUOTE(JSON_EXTRACT(CAST(he.data AS JSON), '$.note')) AS CHAR(1000) CHARACTER SET utf8mb4) COLLATE utf8mb4_unicode_ci FROM " + CommonView.DataBase + ".history_entry he WHERE he.orderId=O.Id AND he.type='ORDER_NOTE' ORDER BY he.updatedAt DESC LIMIT 1), CAST(O.customFieldsOtherinstructions AS CHAR(1000) CHARACTER SET utf8mb4) COLLATE utf8mb4_unicode_ci, CAST('' AS CHAR(1000) CHARACTER SET utf8mb4) COLLATE utf8mb4_unicode_ci) COLLATE utf8mb4_unicode_ci as PaymentMode,customFieldsOtherinstructions as CuttingInstructions,1 as OrdNo,CustomerId,'' as Pincode,py.metadata,py.method as PaymentMethod,'' as PaymentStatus ,";
                    qry += "O.customFieldsLoyaltypointsused AS RewardPointUsed,";
                    qry += "O.couponCodes AS CouponCode";
                    qry += " FROM " + CommonView.DataBase + ".order O," + CommonView.DataBase + ".payment py," + CommonView.DataBase + ".order_line OL," + CommonView.DataBase + ".product_variant_translation P," + CommonView.DataBase + ".stock_movement S," + CommonView.DataBase + ".shipping_line sl," + CommonView.DataBase + ".shipping_method_translation st, ";
                    ...
                    qry += " Union All";
                    // Old row: qry += " SELECT O.iD,SKU AS PCODE,DENSE_RANK() OVER (ORDER BY Code) as SNo,CODE,CAST(O.updatedAt AS DATE)Date,CAST(O.updatedAt AS TIME)TIME,ShippingAddress as BillingAddress,'' as Customer,'' as Address,'' as CellNo,trim(p.NAME) as ItemName,'' as TamilName,count(LineId) as Qty,s.ListPrice/100 as Rate, (Count(LineId)*(s.Listprice/100)) as as Total,subTotalWithTax/100 as SubTotal,shippingWithTax/100 as Shipping,(subTotalWithTax+shippingWithTax)/100 as NetTotal,st.Name as DeliveryTime,COALESCE((SELECT CAST(JSON_UNQUOTE(JSON_EXTRACT(CAST(he.data AS JSON), '$.note')) AS CHAR(1000) CHARACTER SET utf8mb4) COLLATE utf8mb4_unicode_ci FROM " + CommonView.DataBase + ".history_entry he WHERE he.orderId=O.Id AND he.type='ORDER_NOTE' ORDER BY he.updatedAt DESC LIMIT 1), CAST(O.customFieldsOtherinstructions AS CHAR(1000) CHARACTER SET utf8mb4) COLLATE utf8mb4_unicode_ci, CAST('' AS CHAR(1000) CHARACTER SET utf8mb4) COLLATE utf8mb4_unicode_ci) COLLATE utf8mb4_unicode_ci as PaymentMode,customFieldsOtherinstructions as CuttingInstructions,2 as OrdNo,CustomerId,'' as Pincode,py.metadata,'' as PaymentStatus,";
                    // New row: Added py.method as PaymentMethod column (Line 459)
                    qry += " SELECT O.iD,SKU AS PCODE,DENSE_RANK() OVER (ORDER BY Code) as SNo,CODE,CAST(O.updatedAt AS DATE)Date,CAST(O.updatedAt AS TIME)TIME,ShippingAddress as BillingAddress,'' as Customer,'' as Address,'' as CellNo,trim(p.NAME) as ItemName,'' as TamilName,count(LineId) as Qty,s.ListPrice/100 as Rate, (Count(LineId)*(s.Listprice/100)) as as Total,subTotalWithTax/100 as SubTotal,shippingWithTax/100 as Shipping,(subTotalWithTax+shippingWithTax)/100 as NetTotal,st.Name as DeliveryTime,COALESCE((SELECT CAST(JSON_UNQUOTE(JSON_EXTRACT(CAST(he.data AS JSON), '$.note')) AS CHAR(1000) CHARACTER SET utf8mb4) COLLATE utf8mb4_unicode_ci FROM " + CommonView.DataBase + ".history_entry he WHERE he.orderId=O.Id AND he.type='ORDER_NOTE' ORDER BY he.updatedAt DESC LIMIT 1), CAST(O.customFieldsOtherinstructions AS CHAR(1000) CHARACTER SET utf8mb4) COLLATE utf8mb4_unicode_ci, CAST('' AS CHAR(1000) CHARACTER SET utf8mb4) COLLATE utf8mb4_unicode_ci) COLLATE utf8mb4_unicode_ci as PaymentMode,customFieldsOtherinstructions as CuttingInstructions,2 as OrdNo,CustomerId,'' as Pincode,py.metadata,py.method as PaymentMethod,'' as PaymentStatus,";
                    qry += "O.customFieldsLoyaltypointsused AS RewardPointUsed,";
                    qry += "O.couponCodes AS CouponCode";
                    qry += " FROM " + CommonView.DataBase + ".order O," + CommonView.DataBase + ".payment py," + CommonView.DataBase + ".order_line OL," + CommonView.DataBase + ".product_variant_translation P," + CommonView.DataBase + ".stock_movement S," + CommonView.DataBase + ".shipping_line sl," + CommonView.DataBase + ".shipping_method_translation st, ";
                    qry += CommonView.DataBase + ".product_variant_price op," + CommonView.DataBase + ".product_variant PV WHERE O.ID=OL.ORDERID and py.OrderId=o.Id AND OL.PRODUCTVARIANTID=P.ID AND S.ORDERLINEID=OL.ID and sl.OrderId=o.Id and sl.shippingmethodid=st.Id and sl.OrderId=o.Id and op.variantid=ol.productvariantid and op.variantid=p.id AND PV.iD=P.ID AND CAST(O.updatedAt AS DATE) between '" + fDt.ToString("yyyy/MM/dd") + "' AND '" + tDt.ToString("yyyy/MM/dd") + "'  AND (O.customFieldsCancellationreason = '' OR O.customFieldsCancellationreason IS NULL) and o.State not in ('DELIVERED','CANCELLED','CancellationRequested') and st.Name='Tomorrow Evening Delivery (Rs.20 incl.Tax)'  and Cast(o.UpdatedAt as Time)>='" + time.ToString("HH:mm:ss") + "'";
        ...
        public DataTable RptLoadCummReport(DateTime fDt, DateTime tDt, string timeType, DateTime time)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(constring1))
            {
                ...
                if (timeType == "ALL")
                {
                    ...
                    ////qry += " FROM `order` O,order_line OL,product_variant_translation P,stock_movement S,shipping_line sl,shipping_method_translation st WHERE O.ID=OL.ORDERID AND OL.PRODUCTVARIANTID=P.ID AND S.ORDERLINEID=OL.ID and sl.OrderId=o.Id and sl.shippingmethodid=st.Id AND CAST(O.updatedAt AS DATE) between '" + fDt.ToString("yyyy/MM/dd") + "' AND '" + tDt.ToString("yyyy/MM/dd") + "' and customFieldsClientrequesttocanceL=0 and o.State not in ('DELIVERED','lLED')";
        ...
        public DataTable RptLoadCummDelTimeReport(DateTime fDt, DateTime tDt, string timeType, DateTime time, int locationId)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(constring1))
            {
                ...
                if (timeType == "ALL")
                {
                    ...
                    if (locationId == 4 || locationId == 6)
{ 
    qry += " AND (st.name LIKE '%Tomorrow Morning Delivery%'  OR st.name LIKE '%Day After Tomorrow%Morning%') "; 
}
                    ...
                    if (locationId == 4 || locationId == 6)
                    {
                        ...
                        if (locationId == 4 || locationId == 6)
                        {
                            qry += " AND (st.name LIKE '%Tomorrow Evening Delivery%' OR st.name LIKE '%Day After Tomorrow%Evening%') ";
                    ...
                    qry += " ORDER BY OrdNo, " +
       " CASE " +
       "  WHEN DeliveryTime LIKE '%Day After Tomorrow%Morning%' THEN 1 " +
       "  WHEN DeliveryTime LIKE '%Tomorrow Morning%' THEN 2 " +
       "  WHEN DeliveryTime LIKE '%Day After Tomorrow%Evening%' THEN 3 " +
       "  WHEN DeliveryTime LIKE '%Tomorrow Evening%' THEN 4 " +
       "  ELSE 5 END, " +
       " Category, ItemName ";
        ...
        public DataTable RptLoadBillDelPartnerReport(DateTime fDt, DateTime tDt, int locationId)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(constring1))
            {
                ...
                { qry += "  (st.Name='Tomorrow Morning Delivery (Rs.50 incl.Tax)' or st.name='Day After Tomorrow – Morning Delivery (Rs.10 incl. tax)') and "; }
                ...
                //if (locationId == 4)
                {
                    ...
                    // New row: Added py.method as PaymentMethod column (Line 865)
                    qry += " SELECT Distinct DENSE_RANK() OVER (ORDER BY Code) as SNo,CODE,Metadata,customFieldsOtherinstructions as CuttingInstructions,COALESCE(JSON_UNQUOTE(JSON_EXTRACT(CAST(he.data AS JSON), '$.note')), '' ) AS PaymentMode,(subTotalWithTax+shippingWithTax)/100 as NetTotal,CAST(O.updatedAt AS TIME) AS Time,ShippingAddress as BillingAddress,2 as OrdNo,CustomerId,'' as Pincode,py.method as PaymentMethod,'' as PaymentStatus,om.note as Note";
                    ...
                    if (locationId == 4)
                    { qry += " (st.Name='Tomorrow Evening Delivery (Rs.20 incl.Tax)' or st.name='Day After Tomorrow – Evening Delivery (Rs.10 incl. tax)') "; }
                    else if (locationId == 6)
                    { qry += " (st.Name='Tomorrow Evening Delivery (Rs.30 incl.Tax)' or st.name='Day After Tomorrow – Evening Delivery (Rs.10 incl. tax)')  "; }
        ...
        public DataTable RptCancelReport(DateTime fDt, DateTime tDt)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(constring1))
            {
                ...
                qry += " Where customFieldsPlacedatistformatted between '" + fDt.ToString("yyyy/MM/dd") + " 00:00:00' AND '" + tDt.ToString("yyyy/MM/dd") + " 23:59:59' and ";
                qry += " (O.customFieldsCancellationreason IS NOT NULL AND O.customFieldsCancellationreason <> '') and orderPlacedAt is not null ";
        ...
        public void RptLoadReport_GridStyle(DataGridViewColumnCollection tmp)
        {
            ...
            tmp.Add(CommonView.GetGridViewColumn("customFieldsInvoiceno", "Invoice No", 100, 33, true));
