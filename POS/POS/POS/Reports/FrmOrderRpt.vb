        ...
        Try
            ...
            If rdbDetail.Checked Then
                ...
                For i As Integer = 0 To ReportTable.Rows.Count - 1
                    ...
                    If ReportTable.Columns.Contains("customFieldsInvoiceno") Then
                        rptLoad.Rows(i)("customFieldsInvoiceno") = ReportTable.Rows(i)("customFieldsInvoiceno")
                    End If
                    ...
                    ' Old row: ' 1️⃣ Razorpay = Bill Paid
                    ' Old row: If meta.Contains("razorpayPaymentId") Then
                    ' Old row:     isBillPaid = True
                    ' Old row: End If
                    ' Old row:
                    ' Old row:
                    ' Old row: If meta.Contains("""mode"":""online""") Then
                    ' Old row:     isBillPaid = True
                    ' Old row: End If
                    ' Old row:
                    ' Old row: Dim p As String = payMode.ToLower().Replace(" ", "").Replace(".", "")
                    ' Old row: If p.Contains("billpaid") Then
                    ' Old row:     isBillPaid = True
                    ' Old row: End If
                    ' Old row: Dim p1 As String = payMode1.ToLower().Replace(" ", "").Replace(".", "")
                    ' Old row: If p1.Contains("billpaid") Then
                    ' Old row:     isBillPaid = True
                    ' Old row: End If
                    ' Old row:
                    ' Old row: If meta.Contains("""paymentmethod"":""offline""") Then
                    ' Old row:     isBillPaid = False
                    ' Old row: End If

                    ' New row: Check PaymentMethod column for "razorpay-payment" (Line 238-245)
                    Dim paymentMethod As String = ""
                    If ReportTable.Columns.Contains("PaymentMethod") Then
                        Dim methodObj As Object = ReportTable.Rows(i)("PaymentMethod")
                        If methodObj IsNot Nothing AndAlso Not IsDBNull(methodObj) Then
                            paymentMethod = methodObj.ToString().Trim().ToLower()
                        End If
                    End If

                    ' New row: Check if payment method is "razorpay-payment" (Line 247-249)
    ...
    Sub Cr_OrderBillDelPartnerRegister()
        ...
        Try
            ...
            For i As Integer = 0 To dtb1.Rows.Count - 1
                ...
                ' Old row: If dtb1.Columns.Contains("CuttingInstructions") Then
                ' Old row:     cutInst = dtb1.Rows(i)("CuttingInstructions").ToString()
                ' Old row: End If
                ' Old row: If dtb1.Columns.Contains("PaymentMode") Then
                ' Old row:     paymode = dtb1.Rows(i)("PaymentMode").ToString()
                ' Old row: End If
                ' Old row:
                ' Old row: If meta <> "" Then
                ' Old row:
                ' Old row:
                ' Old row:     If meta.ToLower().Contains("razorpaypaymentid") Then
                ' Old row:         isBillPaid = True
                ' Old row:     End If
                ' Old row:
                ' Old row:
                ' Old row:     If meta.ToLower().Contains("""mode"":""online""") Then
                ' Old row:         isBillPaid = True
                ' Old row:     End If
                ' Old row:
                ' Old row:
                ' Old row:     If meta.ToLower().Contains("""paymentmethod"":""offline""") Then
                ' Old row:         isBillPaid = False   ' force COD
                ' Old row:     End If
                ' Old row:
                ' Old row: End If
                ' Old row: Dim p As String = cutInst.ToLower().Replace(" ", "").Replace(".", "")
                ' Old row: If p.Contains("billpaid") Then
                ' Old row:     isBillPaid = True
                ' Old row: End If
                ' Old row: Dim p1 As String = paymode.ToLower().Replace(" ", "").Replace(".", "")
                ' Old row: If p1.Contains("billpaid") Then
                ' Old row:     isBillPaid = True
                ' Old row: End If

                ' New row: Check PaymentMethod column for "razorpay-payment" (Line 939-946)
                Dim paymentMethod As String = ""
                If dtb1.Columns.Contains("PaymentMethod") Then
                    Dim methodObj As Object = dtb1.Rows(i)("PaymentMethod")
                    If methodObj IsNot Nothing AndAlso Not IsDBNull(methodObj) Then
                        paymentMethod = methodObj.ToString().Trim().ToLower()
                    End If
                End If

                ' New row: Check if payment method is "razorpay-payment" (Line 948-950)
                If paymentMethod = "razorpay-payment" Then
                    isBillPaid = True
                End If
                Dim payModeCheck As String = ""
